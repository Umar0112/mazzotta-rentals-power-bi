                ...
                return (
                  <td key={colIndex} className={`px-[12px] whitespace-nowrap ${['Units', 'Rental Days', 'UNITS', 'DAYS'].includes(h) ? 'text-center' : ''}`}>
                    {isItemNum && (value === null || value === '' || value === 'OPEN') ? (
                      <span className="inline-flex items-center py-[1px] font-['Inter:Bold',sans-serif] font-bold text-[12px] text-[#c72e23] whitespace-nowrap">OPEN</span>
                    ) : isRep ? (
...
export default function ReservationAndContractView({ type, title }: ReservationAndContractViewProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full overflow-hidden bg-[#f8fafc]">
      {groupedData.summary && (
        <div className="shrink-0 pt-[4px] pb-[10px] px-[24px]">
          <KpiCard
            todayTitle={`Reservations - ${pageType}`}
