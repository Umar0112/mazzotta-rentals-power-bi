        ...
        <thead className="sticky top-0 z-10">
          <tr className="bg-[#f8fafc] shadow-[0_1px_0_0_#f1f5f9]">
            {tableHeaders.map((h, i) => (
              <th key={i} className={`px-[12px] py-[8px] font-['Inter:Bold',sans-serif] font-bold text-[12px] text-gray-600 tracking-[0.7px] uppercase whitespace-nowrap ${['Units', 'Rental Days', 'UNITS', 'DAYS'].includes(h) ? 'text-center' : 'text-left'}`}>
                ...
                return (
                  <td key={colIndex} className={`px-[12px] py-[6px] whitespace-nowrap ${['Units', 'Rental Days', 'UNITS', 'DAYS'].includes(h) ? 'text-center' : ''}`}>
                    {isItemNum && (value === null || value === '' || value === 'OPEN') ? (
                      <span className="inline-flex items-center px-[5px] py-[1px] font-['Inter:Bold',sans-serif] font-bold text-[12px] text-[#c72e23] whitespace-nowrap">OPEN</span>
                    ) : isRep ? (
                      <span className={`inline-flex items-center px-[5px] py-[1px] font-['Inter:Semi_Bold',sans-serif] font-semibold text-[12px] ${cellHasExpired ? 'text-[#c72e23]' : 'text-[#334155]'} whitespace-nowrap`}>{value}</span>
                    ) : isNotes ? (
                      <span
                        className={`font-['Inter:Medium',sans-serif] font-medium text-[13px] ${cellHasExpired ? 'text-[#c72e23]' : 'text-[#475569]'}`}
    ...
    <div className="flex flex-col h-full w-full overflow-hidden bg-[#f8fafc]">
      ...
      <div className="flex-1 min-h-0 flex flex-col gap-[20px] px-[24px] pb-6 no-scrollbar overflow-y-auto">
        ...
        {groupedData.reservations.length === 0 && groupedData.contracts.length === 0 && (
          <div className="flex-1 min-h-0 flex flex-col rounded-[16px] items-center justify-center p-12 ">
